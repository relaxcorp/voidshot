...
for i in $(seq 1 100); do
  ...
  if [ -n "$ART" ]; then
    ...
    curl -sL -H "Authorization: token $TOKEN" "$API/repos/$OWNER/voidshot/actions/artifacts/$ART/zip" -o "$OUT/art11.zip"
    rm -rf "$OUT/art11"; mkdir -p "$OUT/art11"
    (cd "$OUT/art11" && unzip -o ../art11.zip >/dev/null 2>&1)
    echo "содержимое:"; find "$OUT/art11" -type f -printf '  %s bytes  %p\n'
    ...
    for f in "voidshot.exe" "bundle/nsis/Voidshot_0.1.0_x64-setup.exe" "bundle/msi/Voidshot_0.1.0_x64_en-US.msi"; do
      nm=$(basename "$f")
      code=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: token $TOKEN" -H "Content-Type: application/octet-stream" --data-binary @"$OUT/art11/$f" "$UP/repos/$OWNER/voidshot/releases/$RID/assets?name=$nm")
      echo "  upload $nm: $code"
    done

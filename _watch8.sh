...
for i in $(seq 1 100); do
  ...
  if [ -n "$ART" ]; then
    ...
    curl -sL -H "Authorization: token $TOKEN" "$API/repos/$OWNER/voidshot/actions/artifacts/$ART/zip" -o "$OUT/art8.zip"
    rm -rf "$OUT/art8"; mkdir -p "$OUT/art8"
    (cd "$OUT/art8" && unzip -o ../art8.zip >/dev/null 2>&1)
    echo "содержимое:"; find "$OUT/art8" -type f -printf '  %s bytes  %p\n'
    ...
    for f in "voidshot.exe" "bundle/nsis/Voidshot_0.1.0_x64-setup.exe" "bundle/msi/Voidshot_0.1.0_x64_en-US.msi"; do
      nm=$(basename "$f")
      code=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: token $TOKEN" -H "Content-Type: application/octet-stream" --data-binary @"$OUT/art8/$f" "$UP/repos/$OWNER/voidshot/releases/$RID/assets?name=$nm")
      echo "  upload $nm: $code"
    done

...
for i in $(seq 1 100); do
  ...
  if [ -n "$ART" ]; then
    ...
    curl -sL -H "Authorization: token $TOKEN" "$API/repos/$OWNER/voidshot/actions/artifacts/$ART/zip" -o "$OUT/art5.zip"
    rm -rf "$OUT/art5"; mkdir -p "$OUT/art5"
    (cd "$OUT/art5" && unzip -o ../art5.zip >/dev/null 2>&1)
    echo "содержимое:"; find "$OUT/art5" -type f -printf '  %s bytes  %p\n'
    ...
    for f in "voidshot.exe" "bundle/nsis/Voidshot_0.1.0_x64-setup.exe" "bundle/msi/Voidshot_0.1.0_x64_en-US.msi"; do
      nm=$(basename "$f")
      code=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: token $TOKEN" -H "Content-Type: application/octet-stream" --data-binary @"$OUT/art5/$f" "$UP/repos/$OWNER/voidshot/releases/$RID/assets?name=$nm")
      echo "  upload $nm: $code"
    done
